...
def make_judge(llm_config: dict) -> Judge:
    ...
    if backend == "ollama":
        ...
        return OllamaJudge(model or "qwen3:4b", base_url=base_url)
    if backend == "deepseek":
        from judge.deepseek import DeepSeekJudge

        return DeepSeekJudge(model or "deepseek-chat", api_key=api_key, base_url=base_url)
    if backend == "anthropic":
        from judge.anthropic import AnthropicJudge

        return AnthropicJudge(model or "claude-haiku-4-5", api_key=api_key, base_url=base_url)
    if backend == "openai":
        from judge.openai import OpenAIJudge

        return OpenAIJudge(model or "gpt-4o-mini", api_key=api_key, base_url=base_url)
