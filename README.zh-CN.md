...
| 🔒 **本地优先** | `~/.chief` 下一个 SQLite 文件加几份 markdown。没有托管服务和遥测；本机控制台只监听 `127.0.0.1`。 |
...
你给的 👍/👎("值得打扰"/"别烦我")是奖励信号,按主题的 EMA 权重是策略,反馈训练它们。单用户 eval(`chief eval --learning`)证明这个闭环能收敛;但一个用户只是轶事,所以还有一个**队列基准**:`chief eval --cohort` 在 **100 个模拟用户**上跑同一个闭环(种子固定、已提交的数据集 `eval/personas.jsonl`,各有隐藏偏好、不同场景、不同反馈噪声),再在**留出**事件流上评打断质量:
...
64% 的用户在中位 3 轮内达到 ≥95% 一致率;剩下 36% **恰好**是那些"想要但太安静"的主题被有界权重证明无法抬过其场景阈值的用户(强度 `s` 的主题最高只能到 `5s²`,只有当 `s ≥ √(T/5)` 时才越过阈值 `T`)——一个用户收敛 **当且仅当**其每个想要的主题都可达,于是 `converged ∪ ceiling-capped == 全体`,并有测试钉死。完整推导(三个走查用户)见 **[docs/eval/cohort-benchmark.md](docs/eval/cohort-benchmark.md)**。

> **这为什么重要:** 大多数"它会学你的偏好"只是一个无法证伪的 demo。这个基准既给出人群分布,又给出反馈失效的**确切边界**——能说清楚你的系统在哪里失效,比声称它从不失效更有说服力。

## 🔬 用数据说话,而非声称

这个页面上的每一个数字,背后都有一个**确定性、离线**的评测,你可以自己跑——不需要密钥、不需要网络。不能被度量的主张,不上。

| 评测 | 证明什么 | 怎么跑 |
|---|---|---|
| **路由回归** | 演示的 24 个事件**精确**按钉死的路由走——CI 门禁,永远 100% | `chief eval` |
| **能力(capability)** | 在 **200 条黄金数据集**上的一致率,标签对真实流水线逐一校验过 | `chief eval` |
| **奖励闭环** | ±1 反馈训练策略——单用户 2 轮内 0% → 100% | `chief eval --learning` |
| **队列基准** | 泛化到 **100 个用户**:64% 收敛,留出集打断 **F1 0.10 → 0.81**,且天花板**可证明** | `chief eval --cohort` |
| **成本核算** | 每个决策带按模型、缓存感知的美元成本——抓到过一个 **17× 计价** bug | `chief trace <id>` |
| **优雅降级** | 裁判离线 → 纯规则保守路由,失明期间绝不打断,自动痊愈 | 故障注入测试 |
| **提示词治理** | 任何提示词变更都必须附黄金集上的 eval diff 才能合入 | `chief eval --compare v1 v2` |

**341 个测试,全部离线。** 演示的路由表是一张全表回归——每个事件的路由都被钉死,任何行为变化都无法悄悄溜过去。

```bash
make test lint      # pytest + ruff,无需密钥
make demo           # 离线的"工程师的一天"回放
make release-check  # 构建 wheel,再用 uvx 从 wheel 里跑演示
```
...
`chief ui`(或常驻的 `chief run`)在 `http://127.0.0.1:8787/ui` 提供本地控制台——单用户、token 门禁、无云端:**Today**(摘要队列与花销一览)、**History**(每个决策的理由/得分/成本,可搜索)、**Rules**(直接编辑 POLICY.md,即刻生效)、**Tasks**(批准/拒绝派发的任务,核验照旧)、以及每条决策上的 **👍值得打扰 / 👎别烦我**——学习器把这两个信号看得比一切推断信号都重。

## 🔌 开箱即用的信源

```bash
chief connect composio --secret whsec_…   # GitHub、Gmail、Slack 等 500+ 应用
chief connect github                      # gh 通知轮询
chief connect rss --url https://hnrss.org/frontpage
chief sources                             # 看看接了什么
```

**[Composio](https://composio.dev)** 是旗舰连接器:把它的 trigger webhook 指向 `/v1/connectors/composio`(HMAC 验签),所有已连接应用都流经 Chief 的判断层。注册表为 zapier/n8n 类自动化和 MCP 推送智能体留好了插槽——每个只需一个适配模块。
...
[ROADMAP.md](ROADMAP.md) 列出了 v1 刻意不做的事(托管或多用户 Web UI、云同步、
...
没有托管服务、没有遥测、没有任意 shell 执行。本机控制台只监听
`127.0.0.1`;其他网络请求都是你自己配置的(你的 LLM 后端、你的 Telegram bot)。
