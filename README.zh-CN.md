...
64% 的用户在中位 3 轮内达到 ≥95% 一致率;剩下 36% **恰好**是那些"想要但太安静"的主题被有界权重证明无法抬过其场景阈值的用户——`converged ∪ ceiling-capped == 全体`,并有测试钉死。完整推导(三个走查用户 + `s ≥ √(T/5)` 天花板)见 **[docs/eval/cohort-benchmark.md](docs/eval/cohort-benchmark.md)**。
